...
abstract class BaseStock extends BaseObject  implements Persistent
{
  ...
  const PEER = 'StockPeer';
	...
	public function setBalance($v)
	{
		...
		if ($this->balance !== $v) {
			$this->balance = $v;
			$this->modifiedColumns[] = StockPeer::BALANCE;
		}
	...
	public function setUpc($v)
	{
		...
		if ($this->upc !== $v) {
			$this->upc = $v;
			$this->modifiedColumns[] = StockPeer::UPC;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->balance = ($row[$startcol + 2] !== null) ? (double) $row[$startcol + 2] : null;
			$this->created = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->upc = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = StockPeer::NUM_COLUMNS - StockPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setUpc($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(StockPeer::BALANCE)) $criteria->add(StockPeer::BALANCE, $this->balance);
		if ($this->isColumnModified(StockPeer::CREATED)) $criteria->add(StockPeer::CREATED, $this->created);
		if ($this->isColumnModified(StockPeer::UPC)) $criteria->add(StockPeer::UPC, $this->upc);
